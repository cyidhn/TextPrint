<template>
	<v-row justify="center">
		...
		<!-- /Modal ajouter un texte -->
		<v-dialog v-model="data.lemmes" persistent scrollable max-width="500px">
			<v-card>
				<v-card-title>Lemmatisation</v-card-title>
				<v-divider></v-divider>
		...
		methods: {
			...
			validate() {
				if (this.$refs.form.validate()) {
					...
					axios
						.post(
							process.env.VUE_APP_SERVEUR + "/traitement-lemmatisation",
							formData
						)
						.then((response) => {
							// begin
