<template>
	<v-row justify="center">
		...
		<!-- /Modal ajouter un texte -->
		<v-dialog v-model="data.ngpos" persistent scrollable max-width="500px">
			<v-card>
				<v-card-title>N-grammes de catégories grammaticales</v-card-title>
				<v-divider></v-divider>
		...
		methods: {
			...
			validate() {
				if (this.$refs.form.validate()) {
					...
					axios
						.post(
							process.env.VUE_APP_SERVEUR + "/ngrams-pos-generate",
							formData
						)
						.then((response) => {
							// begin
